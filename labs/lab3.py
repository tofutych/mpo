...
def Lab_3_window():
    ...
    class GeneticEvolution:
        ...
        def killing(self, population):
            res = np.argsort([self.func(item[0], item[1])
    ...
    def draw():
        ...
        for pn in point:
            ...
            if func != 0.0:
                ...
                print(
                    f"Значение функции = {func} в точке x = {pn[0]}, y = {pn[1]}\n")

            else:
                ax.scatter(pn[0], pn[1], func, c="red", s=3)
                print(
                    f'Наименьшее значение функции = {func} в точке x={pn[0]}, y={pn[1]}')
    ...
    window_lab_3.wm_title(
        "Третья лабораторная. Генетический алгоритм оптимизации функции Розенброкка.")
