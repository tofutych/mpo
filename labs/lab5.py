...
def Lab_5_window():
    ...
    def draw():
        ...
        for i in range(0, int(maxIter)):
            ...
            if i != int(maxIter) -1:
                ...
                print(f"Значение функции = {value} в точке x = {pn[i][0]}, y = {pn[i][1]}\n")
            else:
                ax.scatter(pn[i][0], pn[i][1], value, c="red", s=10)
                print(f'Наименьшее значение функции = {value} в точке x={pn[i][0]}, y={pn[i][1]}')
                break
            canvas.draw()
            window_lab_5.update()
            delay = txt_5.get()
    ...
    window_lab_5.wm_title("Пятая лабораторная. Пчелиный алгоритм оптимизации функции Растригина.")
