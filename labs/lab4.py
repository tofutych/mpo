...
def Lab_4_window():
    ...
    def draw():
        ...
        for pn in point:
            if point[::-1].index(pn) != 0:
                ...
                print(f"Значение функции = {pn[2]} в точке x = {pn[0]}, y = {pn[1]}\n")
            else:
                ax.scatter(pn[0], pn[1], pn[2], c="red", s=10)
                print(f'Наименьшее значение функции = {pn[2]} в точке x={pn[0]}, y={pn[1]}')
                break
            canvas.draw()
            window_lab_4.update()
            delay = txt_5.get()
    ...
    window_lab_4.wm_title("Четвёртая лабораторная. Алгоритм роя частиц оптимизации функции Растригина.")
